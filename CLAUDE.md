# beeepbeep-admin — Dashboard d'administration
...
- **Schema** : fichier local `./schema.graphql` (69KB) — copié manuellement depuis `bo-v2`
...
Quand le schéma `bo-v2` change : copier le nouveau `schema.graphql` à la racine puis `npm run codegen`. Ou utiliser `/sync-graphql`.
...
Pattern : remplacer un cast `as Foo` aveugle par `parseFoo(input)` qui renvoie `null` + warn si shape invalide. Aligné avec le pattern de `beepbeepcity-app/validation/`.
...
- Si une query renvoie `null` pour un champ existant en DB, vérifier les `accesses.ts` côté `bo-v2` — la query passe mais le champ est filtré

## Commandes Claude utiles dans ce projet

- `/sync-graphql` après une modif schéma `bo-v2`
