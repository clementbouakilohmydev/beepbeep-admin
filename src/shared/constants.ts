 ...
 * conducteur — modèle Uber : un compte à rebours court, puis plus d'annulation
 * possible du tout.
 *
 * Pendant la fenêtre : annulation libre et gratuite (demande lancée par erreur,
 * mauvaise adresse). Passé la fenêtre : le serveur refuse. Le conducteur est
 * alors certain que sa course tient, ce qui était l'objectif — il a engagé du
 * temps et de l'essence pour rejoindre le point de rendez-vous.
 *
 * Le CONDUCTEUR, lui, n'est jamais bloqué (arbitrage client du 13/08/2026) :
 * il ne doit pas être contraint de rouler du seul fait d'avoir accepté.
 *
 * ⚠️ Un passager bloqué dont le conducteur ne vient jamais n'a plus aucune
 ...
 *
 * Volontairement DISTINCTE de `PASSENGER_CANCEL_WINDOW_SECONDS`, même si les
 * deux valent 60 s aujourd'hui. Tant qu'elles sont égales, aucun frais n'est
 * jamais prélevé : le passager ne peut annuler que pendant la période
