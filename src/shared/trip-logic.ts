...
import {
  ...
  PASSENGER_CANCEL_WINDOW_SECONDS,
 ...
 * (arbitrage client du 13/08/2026, alignement sur le modèle Uber) :
 *
 * - **passager** : fenêtre de rétractation courte
 *   (`PASSENGER_CANCEL_WINDOW_SECONDS`) après l'acceptation, puis plus rien.
 *   Le conducteur a engagé du temps et de l'essence, il doit être certain que
 *   sa course tient.
 * - **conducteur** : jamais bloqué. Il ne doit pas être contraint de rouler du
 *   seul fait d'avoir accepté (panne, imprévu, passager absent). Les abus se
 *   traitent par la réputation, pas par le verrouillage.
 *
 * `role` est optionnel pour ne pas casser les appelants existants, mais le
 * défaut est le comportement PASSAGER — le plus restrictif. Mieux vaut cacher
 * un bouton à tort que laisser le serveur refuser après coup.
 */
 ...
 * fermée. Sert à afficher le compte à rebours — sans lui l'utilisateur voit un
